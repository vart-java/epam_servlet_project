...
<h1 class="text-center"> Please choose your procedure <span
            ...
            <form action="/main" method="post">
                <select class="form-select form-select-lg mb-3" aria-label=".form-select-lg example" name="master" >
                    <c:forEach var="masterMas" items="${requestScope.masters}">
                        <option value=${masterMas.login}><c:out value="${masterMas.specialization.name}"/></option>
                    </c:forEach>
                </select>
                <input type="date" value=${requestScope.dateNow1}
                        min=${requestScope.dateNow1} max=${requestScope.dateNow7} name="dateAppointment">
                <button type="submit" class="btn btn-primary" name="command" value="regToApp" <c:if test="${user.role.name().equals('GUEST')}">disabled</c:if>>submit</button>
            </form>
        </div>
    </div>
</div>
